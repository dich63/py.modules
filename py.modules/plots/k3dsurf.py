...
def _k3D_surf(Farray,Xarray,Yarray,axes, color_map,Xmm,Ymm,title=None):    
   ...
   #plt_surface.model_matrix=[[1.,  0.,  0.,  0.],
    ...
    if not title is None:
        plt_text=k3d.text2d(text=title, position=(0.5,0),reference_point='ct',label_box=True);
        plot += plt_text;
    '''
...
def surf(F=None,XY=None,axes=['X', 'Y', 'F'], cmp=cmps.jet,Xmm=None,Ymm=None, title=None ):
    ...
    return _k3D_surf(F,X,Y,axes, cmp,Xmm,Ymm,title=title);