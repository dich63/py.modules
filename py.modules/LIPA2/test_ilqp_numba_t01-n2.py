...
ef=lambda t:  np.exp(1j*fr.imag*t)
...
plt.plot(ttx,fu(GFt0(ttx)*ef(ttx))+sh,label='$G^{ex}_{full}$')
plt.plot(ttx,fu(jG(ttx)*ef(ttx)),label='$Spline_{t}$')
#plt.plot(tt,fu(y), marker='o',linewidth=0)
plt.plot(tt,fu(yj*ef(tt)), marker='o',linewidth=0,label='$Jet_{t}$')
