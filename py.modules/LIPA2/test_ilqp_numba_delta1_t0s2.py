...
plt.plot(ttx0,fu(jG(ttx0)),label='$spline_{real}$')

plt.plot(ttx0,sh+fu(GF(ttx0)).imag,label='$G^{ex}_{imag}$')
plt.plot(ttx0,sh+fu(jG(ttx0)).imag,label='$spline_{imag}$')
#plt.plot(tt,fu(y), marker='o',linewidth=0)
plt.plot(tt,fu(yj), marker='o',linewidth=0,label='$jet_{real}$')
plt.plot(tt,sh+fu(yj).imag, marker='o',linewidth=0,label='$jet_{imag}$')
plt.plot([t0,dt],[0,0], marker='x',linewidth=0)
'''
'''
...
#errj=norm(x[5:]-yj[5:])/norm(x[5:]);
f=tt>0.1
errj=norm(x[f]-yj[f])/norm(x[f]);
printf('errorj=%3.2e%%\n',100*errj)
ff=ttx>0.2
gfj=GF(ttx[ff]);
spj=jG(ttx[ff]);
...
#plt.title(sprintf('errorj=%3.1e%% $error_{spline}$=%3.1e%%\n',100*errj,100*errsp))
plt.title(sprintf('errorj=%3.1e%% $error_{spline}$=%3.1e%%\n',100*errj,100*errsp),fontsize=20)