...
[p,d,q,sq]=sym_reduce_ex([c0],[],[a0,a1,a2]);
toc('reduce:')
...
[GF,exg]=ilaplace_functor(p,jo,nd=nd)
...
jet0=jet_csr_klu_number(DC,D=D).reset(dt=dt,LM=LM);

ssc=source_singular_csr(sing)
...
#plt.plot(ttx,fu(GF(ttx))); 
#plt.plot(ttx,fu(GF(ttx).imag))
#plt.plot(tt,fu(y), marker='o',linewidth=0)
plt.plot(tt,fu(yj), marker='o',linewidth=0)
plt.plot(tt,fu(yj.imag), marker='o',linewidth=0)
plt.show()
#print(x)    
#print(y[0])    
