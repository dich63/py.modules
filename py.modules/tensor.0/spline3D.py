...
class tesor_spline_3D(object):
    ...
    def __init__(this,f,X,Y,Z,faxis=(0,1,2),outval=np.nan):
        
        t_=this
        t_.outval=outval;

        
        t_.XYZ=(X,Y,Z)=[ (lambda a : np.array(a,dtype=np.float64).flatten()  )(t)  for t in (X,Y,Z) ]
...
if __name__=='__main__':
    ...
    tt=np.linspace(2.1,45, num=128);
    tz=np.linspace(2.3,23.4, num=5);
