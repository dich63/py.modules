...
import ltx.js as j
js=j.jscript()
oc=js.functors_tree('oc=require("oe").OctaveEngine()',call='feval')
jsi=j.jscript(f_ipc=1)
oci=jsi.functors_tree('oc=require("oe").OctaveEngine()',call='feval')

#a=np.random.rand(int(1024/8),1024,1024)
N=1
#a=np.random.rand(int(1024/8),1024,1024)
a=np.random.rand(int(1024/4)*1024*1024)
jc.ipc_cache_clear()
jc.ipc_mode(0)
