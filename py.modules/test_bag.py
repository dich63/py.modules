...
if __name__=='__main__':
    ...
    from midmeshgen.MID_decay_3B import make_barriers_3B,get_MID_decay_3B,delta
    ...
    for n in range(N):
        ...
        jst1=get_MID_decay_3B(b,count=count,dt=dt,fbc_mask=fbc_mask,sigma_mu_bg=smbg,mt=1,bag=n)
        jjj.append(jst1)
        ...
        #
        plt.plot(tt,jj, color=color[n],label=bago[n])
        #plt.plot(jj, c=c,label=str(fbc_mask))
    plt.legend()
    tit='err='+str(delta(jjj[0],jjj[1]))
    plt.yscale('log')
    #    plt.xscale('log')
    plt.title(tit)
    print('show...');
