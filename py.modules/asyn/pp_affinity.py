...
if sys.platform in ('win32',):
    ...
    def assign_process_to_job(hjob,pid=0):
        ...
        if pid == 0:
            hp = win32process.GetCurrentProcess()
        else:
            flags=win32con.PROCESS_SET_QUOTA | win32con.PROCESS_TERMINATE
            hp=win32api.OpenProcess(flags, 0, pid)
    ...
    def _get_handle_for_pid(pid, ro=True):
        if pid == 0:
            ...
            if not ro:
                flags |= win32con.PROCESS_SET_INFORMATION | win32con.PROCESS_QUERY_INFORMATION
            try:
